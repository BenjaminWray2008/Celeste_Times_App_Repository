...
def query(query, type, args=()):  # Function to run all queries
...
def sob_adder(user_id):  # Add SOB for each category
    ...
    for category in list(sob_dict.keys())[1:]:
        ...
        sob = query('''SELECT SUM(time) AS sob FROM Run WHERE user_id = ?
                    ...
                    'fetchone', (user_id, category))[0]

        category_id = query('SELECT id FROM Category WHERE name = ?',
                            'fetchone', (category,))[0]
        ...
        if not sob_dict[category]:
            sob_dict[category].append((
                ...
                'N/A'))
            # If not matched, their time is invalid (Not completed)

        # For each category get the sum of a users IL times in it
        sob = query('''SELECT SUM(time) AS sob FROM Run WHERE user_id = ?
                    AND type = "IL"
                    AND category_id = ?''',
                    'fetchone', (user_id, category_id))[0]
...
def format_time_normal_form(time):  # Turn time into mm:ss.msmsms
    if float(time) != 0:
        minutes = float(time)//60  # Remainder 60s
...
def format_time_readable_form(time):  # Formatting the time
    ...
    if time != 0:
        time = str(time)
        time_list = re.split('[.:]', time)  # Splitting time into list segments
        minutes = time_list[0]
        seconds = time_list[1]
        ms = time_list[2]
        hours = None
        ...
        if hours:
            time = f"{hours}:{minutes}:{seconds}.{ms}"
...
def valid_time_checker(time):  # Checking if entered time is valid format
    # Valid format should be mm:ss.msmsms
    ...
    try:
        ...
        before_period = time_list[0]  # if valid time this is minutes+seconds
        after_period = time_list[1]  # if valid time this is milliseconds
        ...
        if ':' in time:
            ...
            if (len(str(before_period)) < 3 and
                len(str(before_period)) > 0 and
                    str(before_period).isdigit() and int(before_period) < 60):
                pass
...
def new_user_data(id):  # Populating database with entries for new user
    ...
    for category in results:  # for all categories
        ...
        chapter_listy = []
        checkpoints_list = query('''
                                 SELECT checkpoint_id
                                 FROM CategoryCheckpoint
                                 WHERE category_id = ?''',
                                 'fetchall', (category_id,))
        # get all checkpoints from each categories

        for checkpoint in checkpoints_list:
            results = query('SELECT name FROM Checkpoint WHERE id = ?',
                            'fetchone', (checkpoint[0],))
            name = results[0]

            chapter_name = (name.split(' '))[-1]
            if chapter_name not in chapter_listy:
                # Getting all the chapters within that category
                chapter_listy.append(chapter_name)
            results = query('SELECT id FROM Chapter WHERE name = ?',
                            'fetchone', (chapter_name,))
            chapter_id = results[0]

            # For every checkpoint in the category add an entry for new user
            query('''
                  INSERT INTO Run (
                  time, run_number, type,
                  chapter_id, user_id, category_id)
                  VALUES (0, ?, ?, ?, ?, ?)''',
                  'commit', (checkpoint[0], 'checkpoint',
                             chapter_id, id, category_id))

        for chapter in chapter_listy:
            chapter_id_real = query('SELECT id FROM Chapter WHERE name = ?',
                                    'fetchone', (chapter,))[0]

            # For every chapter in the category add an entry for new user
            query('''
                  INSERT INTO Run
                  (time, run_number, type,
                  chapter_id, user_id, category_id)
...
def data_dictionary_creation(user_id, category_id, variable):
    ...
    results = query('''
                    SELECT checkpoint_id, orderer FROM CategoryCheckpoint
                    WHERE category_id = ? ORDER BY orderer ASC''',
                    'fetchall', (category_id,))
    checkpoint_id = [j[0] for j in results]

    # all the chapter ids
    chapters_query = query('''
                           SELECT chapter_id FROM Run
                           WHERE type = "IL"
                           AND user_id = ? AND
                           category_id = ?;''',
                           'fetchall', (user_id, category_id))
    ...
    for index, chapter in enumerate(new_chapters):
        chapter_id = query('SELECT id FROM Chapter WHERE name = ?',
                           'fetchone', (chapter[0],))[0]

        # Get all the IL times and add to the dictionary
        IL_time = query('''
                        SELECT time FROM Run WHERE type = "IL"
                        AND user_id = ? AND
                        category_id = ? AND
                        chapter_id = ?''', 'fetchone',
    ...
    for checkpoint in checkpoint_id:
        ...
        checkpoint_name = query('SELECT name FROM checkpoint WHERE id = ?',
                                'fetchone', (checkpoint,))
        # Get the time from that checkpoint
        time = query('''
                     SELECT time FROM Run WHERE user_id = ?
                     AND category_id = ? AND
                     run_number = ? AND
                     type = "checkpoint";''', 'fetchone',
        ...
        data_dictionary[results[0]].append([checkpoint_name[0],
                                            format_time_readable_form(
                                                new_time)])
    if variable:  # if profile need a total time for each chapter
...
def social_grabber(user_id):  # Get the socials from a user
    results = query('''
                    SELECT link, social_name FROM Socials WHERE user_id = ?''',
                    'fetchall', (user_id,))
    return results


def ranker(order_clause, category, type_clause):
    # Get the leaderboard of SOBs for everyone in home page
    ...
    rows = query(f'''
                  SELECT
                  r1.user_id, u.name, u.pfp_path, SUM(r1.time) AS sum_of_bests
                  FROM Run r1
...
def comparison_data(user_id, category_id):  # Info for different user
    # Get all time within category from that user
    data_dictionary_compare = data_dictionary_creation(
        user_id, category_id, False)

    # Get user's SOB
    sob_compare = query('''
                        SELECT SUM(time) AS sob FROM Run WHERE
                        user_id = ? AND
...
def send_comparison_data():
    ...
    results = query('SELECT name FROM category WHERE id = ?',
                    'fetchall', (category_id,))
    if results:  # Could have put invalid category in url
        pass
    else:
        category_id = 1

    user_id = query('SELECT id FROM User WHERE name = ?;',
                    'fetchone', (user_name,))
    if results:  # Could have put invalid user in url
        pass
    else:
        user_id = None
...
def home():
    ...
    # Checks how many category entries have been completed with no times of 0
...
def get_leaderboard():
    ...
    if not name:  # If entered in url with invalid category id
        name = ['Any%']
        category = 1
...
def signup():
    if not session.get('user_id'):  # only can access if not logged in
...
def signin():  # only can access if not logged in
...
def new_user():
    if request.method == 'POST':  # If trying to access through URL
        ...
        # Entering new user into database with relevant profile information

        results = query('SELECT id FROM User WHERE name == ?',
                        'fetchone', (username,))
        id = results[0]
...
def new_pfp(user_id, category_id):  # Add new pfp for user
    if check_session():
        ...
        query('UPDATE User SET pfp_path = ? WHERE id = ?;',
              'commit', (pfp_directory, user_id))
...
def add_socials(user_id, category_id):  # Created new social link
    if check_session():  # if not logged in
        ...
        amount = query('SELECT COUNT(user_id) FROM Socials WHERE user_id = ?',
                       'fetchone', (user_id,))[0]
        # 4 socials max
        ...
        if amount <= 3 and social_link and social_name:
            ...
            # If not breaching max, add the new link
...
def get_times(user_id, category_id):  # Information for get_times page
    if check_session() and (user_id == g.user[0][0]):  # If logged in
        ...
        if not results:  # Invalid category id through URL
            category_id = 1  # default to any% (id = 1)

        date_joined_since = query('SELECT date_joined FROM User WHERE id = ?',
                                  'fetchone', (user_id,))[0]
                    ...
                    if valid_time_checker(time):  # If time correct format
                        ...
                        if results:  # If its a checkpoint run
                            ...
                            query('''
                                  ...
                                  (time, user_id, category_id, checkpoint_id))
...
def profile(user_id, category_id):  # User profile display
    results = query('SELECT name FROM user WHERE id = ?',
                    'fetchall', (user_id,))

    if not results:  # Invalid user_id through URL
        abort(404)

    results = query('SELECT name FROM Category WHERE id = ?',
                    'fetchall', (category_id,))
    if not results:  # Invalid category_id through URL
        abort(404)

    pfp = query('SELECT pfp_path FROM User WHERE id = ?',
                'fetchone', (user_id,))[0]
    ...
    results = social_grabber(user_id)  # get user's socials
    # Get their times for current category
    ...
    sob_compare_user = query('''
                             SELECT SUM(time) AS sob FROM Run
                             WHERE user_id = ?
                             AND type = "IL"
